...
connectAdb ip = do
  -- (_,o) <- procStrict "_adb" ["connect",Text.pack ip] empty
  (_,o) <- procStrict "_adb" ["start-server"] empty
...
installAdb apk = do
  proc "_adb" ["install","-r",format fp apk] empty
  putStrLn "Installed experiment."
  IO.hFlush IO.stdout

uninstallAdb :: IO ()
uninstallAdb = do
  proc "_adb" ["uninstall","edu.colorado.plv.droidstar.experiments"] empty
  clearLog
...
clearLog :: IO ()
clearLog = shell "_adb logcat -c" empty >> return ()

launchExp :: Text -> IO ()
launchExp name = 
  let a = "edu.colorado.plv.droidstar.experiments/." <> name <> "Activity"
  in proc "_adb" ["shell","am","start","-n",a] empty >> return ()

logcatDS :: IO (IO EmuMsg,IO ())
logcatDS = do (_,Just hout,_,ph) <- Proc.createProcess 
                                      (Proc.proc "_adb" ["logcat"]){ Proc.std_out = Proc.CreatePipe}
